...
| **3. Tools** | **Stage 2 baseline PARTIAL-PASS.** | `tool-v0.1.1-intent-classified` passes 3/4 UC-TOOL gates (UC-TOOL-3 fails at 83.92% recall, need 90%). `tool-v0.1.0-budget-bucketed` is catastrophic (3/4 gates fail; +3.67% completion vs baseline's 50.33%); the framework caught the obvious anti-pattern. See `docs/finding-tools-stage2-baseline.md`. v0.1.2 should fix the recall via expanded keyword classifier + multi-category fallback. |
...
| **5. Execution policy** | **Stage 1 wedge picked.** | `docs/opportunity-policy.md` picks Wedge A (policy-comparison benchmark with model x policy interaction baked in) after surveying 10 incumbents (LangGraph, LangChain ReAct, AutoGen, CrewAI, DSPy, Reflexion, Inspect AI, task benchmarks, OpenAI Agents SDK, Magentic-One). Stage 2 plan sketched, not yet committed. |
...
Scorecard: 2 strong (model + memory), 1 Stage 3 ROBUST-PASS (recovery), 2 Stage 2 baseline (prompt PASS + tools PARTIAL-PASS), 1 Stage 1 wedge-picked (policy). **Five of six dimensions have completed Stage 2 or later.** Only execution-policy remains at Stage 1.

### Cross-dimension orchestration (new)

The framework also runs a **cross-dimension experiment** (`experiments/cross_dim_stage2.py`) that walks scenarios through one variant of each of three dimensions simultaneously and measures interaction effects. First finding: dimensions compose **sub-additively** on this workload under multiplicative composition. Combining the best variants from prompt, tools, and recovery dimensions produces a config 12pp WORSE than all-baselines because `tool-v0.1.1-intent-classified`'s 83% recall multiplies through every other dimension. See `docs/finding-cross-dim-interaction.md`. **This is the value proposition of the six-dimension architecture made concrete: only joint experiments surface these interactions, and single-dimension benchmarks alone would have produced a deployment recommendation provably worse than no variants at all.**
