...
| **4. Memory** | **Strong.** | The schema-alignment proxy ran all four stages. The graph-GC opportunity has Stages 1, 2 (PASS), and 3 (PASS) complete. Both case studies live under `dimensions/memory/canonicalization/` and `dimensions/memory/lifecycle/` (migrated from `runner/variants/` and `runner/gc_variants/`; backward-compat shims at the old paths). |
...
| **6. Recovery behavior** | **Stage 1 wedge picked + Stage 2 Day 1-2 done.** Scaffolding plus `docs/opportunity-recovery.md` (Wedge A: recovery-policy benchmark suite), `docs/recovery-stage2-day1-verification.md` (incumbents verified via raw GitHub source), and `fixtures/workloads/w_failure_injection.py` (synthetic deterministic failure-injection workload, 16 tests). RecoveryVariant ABC + noop baseline ship at `runner/dimensions/recovery/`. Days 3-5 (pilot variants, runner, first benchmark) not yet done. |

Scorecard: 2 strong, 1 in-progress (recovery at Stage 2 Day 1-2), 3 scaffolded with stubs. The architecture is real, not aspirational; the recovery dimension is actively advancing through the same four-stage discipline as the two memory case studies. Adding the first real variant to any of the scaffolded dimensions is the same shape as adding a new GC variant (see `docs/six-dimensions-architecture.md` for the four-step recipe).
