...
| **2. Prompt** | **Stage 1 wedge picked.** | `docs/opportunity-prompt.md` picks Wedge A (strategy-comparison benchmark with cost-aware Pareto framing) after surveying 9 incumbents including DSPy. The wedge differentiates from DSPy by comparing prompt strategies across models / costs rather than optimizing within one strategy. Stage 2 plan sketched, not yet committed. |
| **3. Tools** | **Stage 2 Day 1-2 done.** | `docs/opportunity-tools.md` picks Wedge A (tool-set composition benchmark). `docs/tools-stage2-day1-verification.md` verifies Anthropic tool_use API + MCP specification via WebFetch; pricing data (290-675 tool-system-prompt tokens; 100+ tokens per added tool) confirms the cost dimension. `fixtures/workloads/w_tool_selection.py` is the synthetic task-completion workload (17 tests). Days 3-5 (pilot variants, runner, first benchmark) pending. |
...
Scorecard: 2 strong (model + memory), 1 Stage 3 ROBUST-PASS (recovery), 1 Stage 2 Day 1-2 (tools), 2 Stage 1 wedge-picked (prompt + policy). **All six dimensions now actively producing finding docs and benchmark numbers.** The architecture's six-dimension claim is now fully load-bearing on code + tests + finding docs, not just on prose.
