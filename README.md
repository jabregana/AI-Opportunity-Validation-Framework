...
- **Findings**: 30+ documented in `docs/finding-*.md`; four self-corrections (entity-norm tail bias, Graphiti `in_degree==0` diagnosis, Mem0 single-seed variance, Graphiti adapter API mismatch)
...
The framework has caught itself surfacing real problems four times. Each catch is a finding doc in `docs/`, none was silently corrected, and each one demonstrates that the methodology is doing its job. This is what separates the framework from "we shipped a prototype and the demo worked":
