...
- **Findings**: 30+ documented in `docs/finding-*.md`; five self-corrections (entity-norm tail bias, Graphiti `in_degree==0` diagnosis, Mem0 single-seed variance, Graphiti adapter API mismatch, memory-lifecycle product shape killed by market re-scan)
- **Market status (2026-07-10 re-scan)**: the tuned-GC-config product shape for the Mem0 path was commoditized by free packages while the storage-layer gap it addresses remains unfilled by any vendor; the sellable assets are the measurement methodology and a cross-framework governance surface. Full evidence and verdicts: [`docs/finding-market-revalidation-2026-07.md`](docs/finding-market-revalidation-2026-07.md)
...
- **Two opportunities through the framework**: entity-normalization proxy at Stage 4 (substantial-N evidence); agent-memory lifecycle management at Stage 5 for Mem0 (deployable bundle + runbook + multi-seed F1 on single SQuAD archetype, awaiting pilot), Stage 1-2 for Graphiti/Cognee (code-complete v0.2.x variants; first real-Neo4j deletions after the adapter fix; no compliant benchmark yet, pending customer signal)
...
The framework has caught itself surfacing real problems five times. Each catch is a finding doc in `docs/`, none was silently corrected, and each one demonstrates that the methodology is doing its job. This is what separates the framework from "we shipped a prototype and the demo worked":
...
| Graphiti adapter called a graphiti-core API that does not exist; every sweep delete silently failed, and the fake test double carried the same wrong contract, so 512 tests passed on the wrong API (the 0% reductions in correction 2 were this bug, not the `in_degree==0` check) | The phantom signature: uniform 0% across all variants plus 0.000s delete wall time; N=5 real-Neo4j smoke after the fix deleted 13 of 14 candidates | Retracted the "architectural wall" claim; adapter fixed; real-API smoke now required before any adapter-mediated claim counts. See [`docs/finding-graphiti-adapter-revision.md`](docs/finding-graphiti-adapter-revision.md) |
| Tuned-GC-config product shape for the Mem0 path invalidated by market motion (Mem0 shipped relevance-layer decay and expiration in May 2026; at least three free packages shipped lifecycle layers by mid-June; the "memory bloat is billable on Mem0" sales claim failed verification) | Scheduled Stage 1 market re-scan, 2026-07-10, with adversarial verification of every load-bearing claim | Repositioned: sell the measurement (memory-quality benchmarking), not the config; the technical numbers stand. See [`docs/finding-market-revalidation-2026-07.md`](docs/finding-market-revalidation-2026-07.md) |
...
**Cross-framework finding, revised**: end-to-end Graphiti benchmarks (3 scenarios) showed v0.1.x variants returning 0% reduction, originally diagnosed as the `in_degree == 0` orphan-node check rarely triggering on edge-rich graphs ([`docs/finding-graphiti-f1-stage5.md`](docs/finding-graphiti-f1-stage5.md)). That diagnosis was retracted: the 0% was a silent adapter API mismatch, fixed in commit `45f4596` and documented in [`docs/finding-graphiti-adapter-revision.md`](docs/finding-graphiti-adapter-revision.md). With the fixed adapter, the v0.2.x bundle produces real Neo4j deletions (26.4% reduction at N=30; the F1 gate fails at 58.6% on that workload's high cross-component entity sharing, with the fix path documented in the revision doc). Whether v0.1.x reduces on edge-rich graphs is now an open question pending a re-run. The Mem0 numbers above stand; the Mem0 adapter used the correct API throughout. The v0.2.x design (7 layers) is documented at [`docs/opportunity-v0.2.x-graph-topology-gc.md`](docs/opportunity-v0.2.x-graph-topology-gc.md).
...
| Memory lifecycle (Mem0 path) | 5 (real LLM + real adapter; multi-seed F1 on single SQuAD archetype; PARTIAL pending archetype expansion) | `gc-v0.1.8-comprehensive-tuned` + Mem0 adapter + production runbook + CI regression gate | The config itself is no longer sellable (free packages ship equivalents; see the [2026-07 market re-scan](docs/finding-market-revalidation-2026-07.md)). The sellable unit is the measurement: a team paying for a memory-quality benchmark run against their own store, then a measured 30-day pilot |
| Memory lifecycle (Graphiti / Cognee path) | 1-2 (v0.2.x code-complete; first real-Neo4j runs after the adapter fix) | Graphiti adapter fixed (commit `45f4596`) and deleting for real; Cognee adapter needs a structural rewrite; whether v0.1.x fits graph-native is open pending re-run | A compliant v0.2.x benchmark (multi-seed, multi-archetype per [`docs/benchmark-methodology.md`](docs/benchmark-methodology.md)), per [`docs/opportunity-v0.2.x-graph-topology-gc.md`](docs/opportunity-v0.2.x-graph-topology-gc.md). Estimated 5-6 calendar weeks once customer signal arrives |

**Entity-normalization proxy**: plausibly a $1M to $10M ARR business, not a $1B startup. The proxy code is ~50 lines anyone can rewrite. The moat is curated vertical alias maps (pharma, finance, legal) sold as a subscription, plus integration-shim maintenance as upstream APIs evolve. The 2026-07 market re-scan confirmed willingness to pay at five figures (Senzing's pricing floor is $58,560/yr; John Snow Labs sells terminology mapping at about $4,400/mo) and confirmed incumbents own the vertical capability in all three target verticals. The viable shape is one vertical, priced under the incumbents, gated on a paying pilot. The benchmark methodology and the harness itself are the brand asset, not the code.

**Memory lifecycle (Mem0 path)**: repositioned by the [2026-07 market re-scan](docs/finding-market-revalidation-2026-07.md). The tuned config was the planned product; free packages now ship equivalents and Mem0 points users toward them, so the config is inventory, not product. What no vendor and no free package ships is evidence: Mem0's native decay and expiration are relevance-layer only (nothing native reduces stored volume), and none of the free lifecycle packages measure whether they preserve retrieval quality. The sellable unit is the measurement this repo already performs: multi-seed reduction and F1-preservation benchmarking under a compliance standard. The "reduce your Mem0 bill" framing is retired; the claim that stored volume is directly billable on Mem0 failed verification. Remaining engineering gaps unchanged: additional workload archetypes, multi-seed CI on the n=2000 reduction smoke. Remaining commercial gap, reordered: buyer discovery for the measurement offering first, then one customer running a measured pilot in production for 30 days.

**Memory lifecycle (Graphiti / Cognee path)**: one v0.2.x build cycle away from a parallel commercializable offering when customer signal arrives. The seven-layer design ([`docs/opportunity-v0.2.x-graph-topology-gc.md`](docs/opportunity-v0.2.x-graph-topology-gc.md)) is documented, the variants are code-complete with passing unit tests, and post-adapter-fix runs delete for real against Neo4j (26.4% reduction at N=30). What has not been produced: a run that passes the F1 gate, and the full compliance matrix. Estimated 5-6 calendar weeks once a Graphiti or Cognee prospect is on the table.

What does NOT get added next: more dimensions, more six-dimension variants, more statistical machinery, or anything competing with free lifecycle packages or platform-native curation (Letta sleep-time agents, Anthropic Dreams). The framework has enough. What gets added next: buyer discovery for the measurement offering, then **one customer who runs a recommendation in production and reports their actual business outcome.**
...
The Graphiti testing arc (see "Cross-framework finding, revised" above) refined the wedge: v0.1.x targets **flat-memory frameworks** (Mem0); the v0.2.x family ([`docs/opportunity-v0.2.x-graph-topology-gc.md`](docs/opportunity-v0.2.x-graph-topology-gc.md)) targets **graph-native frameworks** (Graphiti, Cognee) with graph-topology signals. Both families share the same product story (Agent Memory Lifecycle Management) and the same configurable-policy approach.

Market status as of 2026-07-10: the wedge survived at the storage layer (no vendor natively reduces stored volume) but closed at the config layer (free packages ship policy bundles). The commercial framing moved from selling the bundle to selling the measurement of what any bundle, free or not, does to retrieval quality. Full re-scan: [`docs/finding-market-revalidation-2026-07.md`](docs/finding-market-revalidation-2026-07.md).
...
The numbers above are estimates derived from the measured benchmarks. Cost ranges reflect the variance the multi-seed methodology exposed. The gap between estimate and verified outcome closes when a customer pilot reports their actual savings. One scoping note from the 2026-07 market re-scan: the storage-cost row applies to self-hosted vector-store infrastructure; the claim that stored volume is directly billable on Mem0's managed platform failed verification and is not part of this translation.
...
1. **A customer pilot for either opportunity.** Until a real team runs a recommended bundle in production for 30 days and reports actual storage savings, latency change, and any incidents, the business-outcome claims are estimates. This is the gating constraint between "research asset" and "product." The [2026-07 market re-scan](docs/finding-market-revalidation-2026-07.md) changed what the pilot sells (measurement first, config second) and added a prior step: buyer discovery on what teams would pay for a memory-quality benchmark run, since no price point for that category surfaced in the verified evidence.
2. **A compliant v0.2.x benchmark against real Graphiti.** The seven-layer design is documented, the variants are code-complete with passing unit tests, and post-adapter-fix runs delete for real (26.4% reduction at N=30). But no run has passed the F1 gate yet, the compliance matrix (multi-seed, multi-archetype) has not been produced, and the Cognee adapter needs a structural rewrite before its numbers can be trusted. The Mem0 path has a deployable bundle today (multi-seed F1 on single SQuAD archetype, awaiting pilot); Graphiti and Cognee customers wait until v0.2.x has gate-passing measured numbers.
